...
const { getPersonByIdService } = require("../services/person.service");
const { getProductByIdService } = require("../services/product.serice");
  ...
  try {
    ...
    for (const element of data.products) {
      ...
      if (!existingProduct) {
        return res
          .status(404)
          .json({ message: " Product Not Found not found " });
        break;
      }
    }
...
const getAllOrderController = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `failed to get all products:${error.message}` });
  }
};
...
const getOrderByIdController = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `failed to get order${error.message}` });
  }
};
...
const updateOrderByIdController = async (req, res) => {
  try {
    ...
    if (!UpdateOrder) {
      return res
        .status(204)
        .json({ message: "no Order found with the given id to update" });
    }
    return res.status(200).json(UpdateOrder);
  } catch (error) {
    return res
      .status(500)
      .json({ message: `failed to update Order:${error.message}` });
  }
};
...
const deleteOrderByIdController = async (req, res) => {
  try {
    ...
    if (order.status !== "CANCELED") {
      return res
        .status(409)
        .json({ message: "Cannot delete order in this status" });
    }
    ...
    return res
      .status(500)
      .json({ message: `failed to delete order:${error.message}` });
  }
};
