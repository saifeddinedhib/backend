...
const signUpController = async(req,res)=>{
    try{
        ...
        if(existingUser){
            return res.status(409).json({message:"user with this email already taken"})
        }
        ...
        const token = jwt.sign(
            {
                id:newPerson._id, email:newPerson.email, role:newPerson.role
            },
...
const signInController = async(req,res,next)=>{
    try{
        ...
        if(!isPasswordValid){
          ...
          const token = jwt.sign(
            {
                id:userExisting._id, email:userExisting.email, role:userExisting.role
            },
            process.env.SECRET_KEY,
            {
               expiresIn :'24h'
            });
             return res.status (201).json({...userExisting, token});
    }
    catch(error){
